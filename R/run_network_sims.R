...
## Pipeline script
  ...
  run_network_sims = function(data.ecoli, data.social, 
...
# If parallel loading is supported, export data, libraries and functions to each cluster
  ...
  if(spatial) {
    ...
    clusterExport (NULL, c('data.ecoli', 'data.social', 
                          'n.simulations', 'weighted.host'),
                  envir = environment())
  }
...
# If using spatial, get the distance matrices
 ...
 if(spatial){
  ...
  host.2014 <- spatial_2014
  host.2015 <- spatial_2015
...
stopCluster(cl)
  ...
  simulations <- lapply(seq_len(n.simulations), function(x){
    ...
    # space network 
 #   dist.2014<-distance_matrix.2014
  #  dist.netlm.2014 <- sna::netlm(ecoli.2014, dist.2014,
   #                               mode = "graph",
    #                              nullhyp = "qapspp",
     #                             reps = 100)
   #MRQAP.2014.results.dist <- extract_MRQAP_coefs(dist.netlm.2014, ecoli.2014)
   #rm(dist.netlm.2014)
